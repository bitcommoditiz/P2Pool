...
class WorkerBridge(worker_interface.WorkerBridge):
        ...
        def compute_work():
            ...
            if bb is not None and bb['previous_block'] == t['previous_block'] and self.node.net.PARENT.POW_FUNC(bitcoin_data.block_header_type.pack(bb)) <= t['bits'].target:
                ...
                t = dict(
                    ...
                    coinbaseflags='',
                    height=t['height'] + 1,
                    #time=bb['timestamp'] + 300, # better way? 600 ICI
                    time=time.time(),
                    transactions=[],
                    transaction_fees=[],
    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        print 'New work for worker! Difficulty: %.06f Share difficulty: %.06f Total block value: %.6f %s including %i transactions %s work_timestamp' % (
            bitcoin_data.target_to_difficulty(target),
            bitcoin_data.target_to_difficulty(share_info['bits'].target),
            self.current_work.value['subsidy']*1e-8, self.node.net.PARENT.SYMBOL,
            len(self.current_work.value['transactions']),
            self.current_work.value['time'],
        )
            ...
            try:
                if pow_hash <= header['bits'].target or p2pool.DEBUG:
                    ...
                    if pow_hash <= header['bits'].target:
                        ...
                        print 'Block header timestamp : %s' % (header['timestamp'])
                        print
            except:
